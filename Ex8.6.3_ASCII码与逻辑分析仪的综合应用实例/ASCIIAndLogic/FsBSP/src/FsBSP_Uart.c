...
****************************************************************** */
void ConfigUART(unsigned long int baud)
{

    SCON = 0x50;		// 8位数据,可变波特率
	AUXR |= 0x01;		// 串口1选择定时器2为波特率发生器
	AUXR |= 0x04;		// 定时器2时钟为Fosc/12,即12T
	T2L = (65535 - ((11059200/4)/baud));		//设定定时初值
	T2H = (65535 - ((11059200/4)/baud)) >> 8;	//设定定时初值
	AUXR |= 0x10;		// 启动定时器2
}
...
{ 
	...
	{  
		UART_SendString("\r /* ====================== */ \r\n");
		UART_SendString("\r /* 你给我了一个: \r");
		UART_SendOneByte(uTemp);// 回传接收到的数据
		UART_SendString("\r\n");

		TI = 1;
		printf("\r I give you it's ASCII 0x%x ",uTemp);
		UART_SendString("\r\n");
		UART_SendString("\r /* ====================== */ \r\n");
