...
{
	if(SPIx->SPI_SSIG == ENABLE)			SPCTL &= ~(1<<7);	//enable SS, conform Master or Slave by SS pin. 目前只支持DISABLE
	...
	if(SPIx->SPI_Mode == SPI_Mode_Slave)	SPCTL &= ~(1<<4);	//slave  目前只支持SPI_Mode_Master
	...
	if(SPIx->SPI_Interrupt == ENABLE)		IE2 |=  (1<<1);     //目前只支持ENABLE
	else									IE2 &= ~(1<<1);
