...
u8 TxBuf,RxBuf;		// 待发送、接收缓冲数据
u8 TxDat,RxDat;		// 发送、接收中间数据
u8 TxCnt,RxCnt;		// 发送、接收采样计数
u8 TxBit,RxBit;		// 发送、接收位计数
bit TxSta,RxSta;	// 发送、接收开始标志位
bit TxEnd,RxEnd;	// 发送、接收结束标志位
...
{
	if(RxSta)			//如果接收置位，则开始接收
	{
		if(RxCnt == 0)	//采样次数为0时开始采样
		{
			RxCnt = 4;	//重新赋值
			if(--RxBit == 0)		// 采样停止位
			{
			...
			{
				RxDat >>= 1;		// 由于先采样低位，所以数据右移
				if(RxIO)			// 接收到数据“1”
					RxDat |= 0x80;	// 直接或“1”
				// 注意，如果接收到“0”，等价于右移的高位补“0”
			}
		}
		RxCnt--;		//采样次数减一，总共是采样4次
	}
	else if(!RxIO)		//端口为低电平，表示为数据的开始位
	{
		RxSta = 1;		//开始采样接收数据
		RxCnt = 4;		//4倍的采样率，因此赋值为4
		RxBit = 9;
	}
	
	if(--TxCnt == 0)
	{
		TxCnt = 4;		//4倍的采样率，因此赋值为4
		if(RxSta)		//判断是否发送，此时开始发送数据
		{
			if(TxBit == 0)			//发送起始位
			{
				TxIO = 0;			//低电平
				TxDat = TxBuf;		//将待发送数据赋值给发送变量
				TxBit = 9;			//发送计数赋值9（8位数据位+1位停止位）
			}
			else
			{
				TxDat >>= 1;		//右移以后，数据位会移动到CY中
				if(--TxBit == 0)	//发送停止位
				{
					TxIO = 1;		//高电平
					RxSta = 0;		//停止发送
