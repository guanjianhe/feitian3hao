...
extern u16 g_uiCnt_Time;	//变量在stc15_timer.c 中定义

bit g_bProgmOK = 0;			//下载检测OK标准位
...
{
	static u8 uIntCnt = 0;  	//被中断的次数，一个周期为2次
	static u8 uProgmCnt = 0;	//下载软件发送下载码的次数，为稳定，多计数几次
	
	uIntCnt++;
	if(uIntCnt == 3)
	{		
		uIntCnt = 1;
		if((g_uiCnt_Time > 800) && (g_uiCnt_Time < 2200))
		{
			g_uiCnt_Time = 0;
			if(uProgmCnt++ > 10)	//接收到了10个下载码，可以确定是下载信号
			{
				uProgmCnt = 0;		//清零
				g_bProgmOK = 1;		//下载确认OK，置位标志位
				INT_CLKO &= 0xbf;   //关闭外部中断4，防止没重启完又进去下载冷启动状态
			}
		}
		else 						//不是下载信号，直接清零
		{
